...
exports.run = async (client, message, args, level) => {
  ...
  client.booru.search(site, args, {limit: 1, random: true}).then(img => {
    if (!img[0]) {
      message.channel.send("No results found.");
      client.logger.warn(`No results found for tags: ${args.join(", ")}`);
      return;
    }
    ...
    const embed = {"embed": {
      ...
      "url": img[0].source || img[0].file_url,
      "image": {"url": img[0].file_url},
      "footer": {"text":`Score: ${img[0].score}`},
      "timestamp": img[0].createdAt.toString()
    }};
    client.logger.log(`${site == "gb" ? "Gelbooru" : "Safebooru"} #${img[0].id} found for tags: ${args.join(", ")}`);
